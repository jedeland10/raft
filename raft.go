...
func (r *raft) send(m pb.Message) {
	...
	if m.Type == pb.MsgVote || m.Type == pb.MsgVoteResp || m.Type == pb.MsgPreVote ||
...
func (r *raft) appliedTo(index uint64, size entryEncodingSize) {
	...
	if r.trk.Config.AutoLeave && newApplied >= r.pendingConfIndex && r.state == StateLeader {
		...
		if err := r.Step(m); err != nil {
			r.logger.Debugf(
				"not initiating automatic transition out of joint configuration %s: %v",
				r.trk.Config,
				err,
			)
		} else {
...
func (r *raft) Step(m pb.Message) error {
		...
		if m.Type == pb.MsgVote || m.Type == pb.MsgPreVote {
			...
			if !force && inLease {
				...
				r.logger.Infof(
					...
					m.Index,
					r.Term,
					r.electionTimeout-r.electionElapsed,
				)
				return nil
			}
		}
	...
	case m.Term < r.Term:
		if (r.checkQuorum || r.preVote) && (m.Type == pb.MsgHeartbeat || m.Type == pb.MsgApp) {
			...
			if m.Index != 0 {
				...
				// about this race.
				r.logger.Infof(
					"%x [term: %d] ignored entry appends from a %s message with lower term [term: %d]",
					r.id,
					r.Term,
					m.Type,
					m.Term,
	...
	case pb.MsgVote, pb.MsgPreVote:
		...
		if canVote && r.raftLog.isUpToDate(candLastID) {
			...
			r.send(
				pb.Message{To: m.From, Term: r.Term, Type: voteRespMsgType(m.Type), Reject: true},
			)
		}
...
func stepLeader(r *raft, m pb.Message) error {
	...
	case pb.MsgProp:
		...
		for i := range m.Entries {
			...
			if cc != nil {
				...
				if alreadyPending {
					failedCheck = fmt.Sprintf(
						"possible unapplied conf change at index %d (applied to %d)",
						r.pendingConfIndex,
						r.raftLog.applied,
					)
	...
	case pb.MsgAppResp:
		...
		if m.Reject {
			...
			// described below.
			r.logger.Debugf(
				"%x received MsgAppResp(rejected, hint: (index %d, term %d)) from %x for index %d",
				r.id,
				m.RejectHint,
				m.LogTerm,
				m.From,
			...
			if pr.MaybeUpdate(m.Index) || (pr.Match == m.Index && pr.State == tracker.StateProbe) {
				...
				case pr.State == tracker.StateSnapshot && pr.Match+1 >= r.raftLog.firstIndex():
					...
					r.logger.Debugf(
						"%x recovered from needing snapshot, resumed sending replication messages to %x [%s]",
						r.id,
						m.From,
						pr,
					)
...
func stepCandidate(r *raft, m pb.Message) error {
	...
	case pb.MsgTimeoutNow:
		r.logger.Debugf(
			"%x [term %d state %v] ignored MsgTimeoutNow from %x",
			r.id,
			r.Term,
			r.state,
			m.From,
...
func stepFollower(r *raft, m pb.Message) error {
	...
	case pb.MsgTimeoutNow:
		r.logger.Infof(
			"%x [term %d] received MsgTimeoutNow from %x and starts an election to get leadership.",
			r.id,
			r.Term,
			m.From,
		)
	...
	case pb.MsgReadIndexResp:
		...
		r.readStates = append(
			r.readStates,
			ReadState{Index: m.Index, RequestCtx: m.Entries[0].Data},
		)
	}
	return nil
}
...
func (r *raft) handleAppendEntries(m pb.Message) {
		...
		if needsDecode > 4 {
			...
			for i := range m.Entries {
				if m.Entries[i].Type == pb.EntryNormal &&
					unicache.IsEncodedData(m.Entries[i].Data) {
					wg.Add(1)
					go func(i int) {
						defer wg.Done()
			...
			for i := range m.Entries {
				if m.Entries[i].Type == pb.EntryNormal &&
					...
					if decoded, ok := r.raftLog.uniCache.DecodeEntry(m.Entries[i]); ok {
						...
						panic(
							fmt.Sprintf(
								"cache decode failed for index %d committed %d with data: %d",
								m.Entries[i].Index,
								r.raftLog.committed,
								m.Entries[i].Data,
							),
	...
	if a.prev.index < r.raftLog.committed {
		r.send(
			pb.Message{
				...
				Commit: r.raftLog.cacheWaterMark,
			},
		)
		return
	}
	...
	r.logger.Debugf(
		...
		r.id,
		r.raftLog.zeroTermOnOutOfBounds(r.raftLog.term(m.Index)),
		m.Index,
		m.LogTerm,
		m.Index,
		m.From,
...
func (r *raft) restore(s pb.Snapshot) bool {
	...
	if r.raftLog.matchTerm(id) {
		...
		last := r.raftLog.lastEntryID()
		r.logger.Infof(
			"%x [commit: %d, lastindex: %d, lastterm: %d] fast-forwarded commit to snapshot [index: %d, term: %d]",
			r.id,
			r.raftLog.committed,
			last.index,
			last.term,
	...
	last := r.raftLog.lastEntryID()
	r.logger.Infof(
		"%x [commit: %d, lastindex: %d, lastterm: %d] restored snapshot [index: %d, term: %d]",
		r.id,
		r.raftLog.committed,
		last.index,
		last.term,
